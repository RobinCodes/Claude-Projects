...
\author{}
\date{13 August 2026}
\thanks{Prepared in collaboration with an artificial-intelligence system (Claude, Anthropic);
see the note on the use of artificial intelligence at the end of this paper.}
...
This paper was prepared in collaboration with an artificial-intelligence system (Claude, Anthropic).
Knuth's theorem on Fibonacci multiplication and the extensions of it in the works cited below are
not ours; what is contributed here---the transport identity (Theorem~\ref{thm:transport}), the
identification of associativity with closure and its closed-form obstruction
(Theorem~\ref{thm:criterion}, \eqref{eq:obstruction}), the phenomena outside the canonical case of
\S\ref{sec:phenomena}, including the failure of upward-closedness for the Lucas system
(Proposition~\ref{prop:lucas}, Observation~\ref{obs:notupset}) and the minimal certificates of
Proposition~\ref{prop:others}, and the verification script of \S\ref{sec:verification}---was found,
drafted and checked in that collaboration.

No result here rests on machine assertion. Every proof is stated so as to be checkable by hand, and
every finite claim is reproducible by running \texttt{verify.py}, whose arithmetic is exact and
which terminates with \texttt{ALL CHECKS PASSED}. The questions raised in the paper are reported as
open exactly as they stand.
